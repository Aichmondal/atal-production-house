...
export const Packages: React.FC = () => {
  ...
  return (
    <section id="packages" className="relative py-24 bg-[#0D0D11] border-t border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
                <div className="flex items-center gap-2.5">
                  <div className={`px-3 py-1 rounded-lg border text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${getCategoryBadgeClass(catGroup.category)}`}>
                ...
                {catGroup.items.map((pkg) => {
                  ...
                  return (
                    <div
                      key={pkg.id}
                      id={`pkg-card-${pkg.id}`}
                      className={`relative rounded-3xl bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] p-6 sm:p-7 border transition-all duration-300 flex flex-col justify-between hover:-translate-y-1 overflow-hidden ${
                        isHighlight
                    ...
                    >
                      ...
                      {/* Top Badge */}
                      {pkg.badge && (
                        <div className="absolute -top-3 right-6 px-3.5 py-0.5 rounded-full bg-[#D4AF37] text-black text-[10px] font-bold uppercase tracking-wider shadow-md">
                          {pkg.badge}
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <div className="my-5 p-4 rounded-2xl bg-black/60 border border-white/5 flex items-baseline justify-between">
                          <span className="text-xs text-neutral-400 uppercase tracking-wider">
                            {t.packages.studioRate}
                          </span>
                          <div className="font-extrabold text-3xl sm:text-4xl text-[#D4AF37] tracking-tight drop-shadow-[0_0_15px_rgba(212,175,55,0.35)]">
                            {pkg.price}
                          </div>
                        </div>
                      ...
                      <a
                        ...
                        className="w-full inline-flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-white/5 hover:bg-[#7C3AED] text-neutral-200 hover:text-white font-semibold text-xs sm:text-sm tracking-wide border border-white/10 hover:border-[#7C3AED] transition-all shadow-sm group"
                      >
        ...
        <div className="mt-16 text-center">
          <div className="rounded-3xl bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] border border-[#D4AF37]/30 p-6 sm:p-8 max-w-4xl mx-auto shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
              ...
              <a
                ...
                className="shrink-0 inline-flex items-center justify-center gap-2.5 px-7 py-3.5 rounded-full bg-[#D4AF37] hover:bg-white text-black font-bold text-sm sm:text-base tracking-wide shadow-lg shadow-[#D4AF37]/20 transition-all hover:scale-105"
              >
